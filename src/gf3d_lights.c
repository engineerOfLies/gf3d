...
{
    memset(&light_manager.ambientLight,0,sizeof(GF3D_Light));
    light_manager.ambientLight.color = gfc_vector4dxyz(gfc_color_to_vector4f(color));
...
GF3D_Light *gf3d_light_new_spot(GFC_Color color, GFC_Vector3D position, GFC_Vector3D direction, float attenuation, float angle)
{
    ...
    if (!light)return NULL;
    light->color = gfc_vector4dxyz(gfc_color_to_vector4f(color));
    gfc_vector3d_copy(light->direction,direction);
...
void gf3d_light_build_ubo_from_list(GF3D_Light_UBO *ubo,GFC_List *lights)
{
    ...
    memset(ubo,0,sizeof(GF3D_Light_UBO));
